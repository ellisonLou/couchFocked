...
def runCouchPotato(options, base_path, args, data_dir = None, log_dir = None, Env = None, desktop = None):
    ...
    # Do db stuff
    db_path = sp(os.path.join(data_dir, 'database'))  # 规范化path
    old_db_path = os.path.join(data_dir, 'couchpotato.db') # 这里为什么不用规范化？
    ...
    config = {
        'use_reloader': reloader, # reloader是什么东西？
    ...
    application = Application(
        [],
        log_function = lambda x: None,  # python 中的匿名函数
